...
struct RenderResources {
  ...
  ImGuiStorage ImageBindGroups; // Resources bind-group to bind the font/image resources to pipeline
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        if (bind_group) {
          ...
          WGPUBindGroup image_bind_group = ImGui_ImplWGPU_CreateImageBindGroup(
              bd->renderResources.ImageBindGroupLayout, (WGPUTextureView)tex_id);
...
static void ImGui_ImplWGPU_CreateFontsTexture() {
  ...
  // Upload texture data
  {
    ...
    wgpuQueueWriteTexture(bd->defaultQueue, &dst_view, pixels, (uint32_t)(width * size_pp * height),
                          &layout, &size);
  }
...
bool ImGui_ImplWGPU_CreateDeviceObjects() {
  ...
  bd->renderResources.ImageBindGroups.SetVoidPtr(
      ImHashData(&bd->renderResources.FontTextureView, sizeof(ImTextureID)), image_bind_group);
