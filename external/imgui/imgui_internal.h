...
IMGUI_API void
ImStrncpy(char *dst, const char *src,
...
ImTextFindPreviousUtf8Codepoint(const char *in_text_start,
                                const char *in_text_curr); // return previous UTF-8 code-point.
IMGUI_API int
ImTextCountLines(const char *in_text,
...
struct IMGUI_API ImDrawListSharedData {
  ...
  float ArcFastRadiusCutoff;    // Cutoff radius after which arc drawing will fallback to slower
                                // PathArcTo()
  ImU8 CircleSegmentCounts[64]; // Precomputed segment count for given radius before we calculate it
...
enum ImGuiItemFlagsPrivate_ {
  ...
  ImGuiItemFlags_MixedValue =
      1 << 12, // false     // [BETA] Represent a mixed/indeterminate value, generally
               // multi-selection where values differ. Currently only supported by Checkbox() (later
               // should support all sorts of widgets)
  ...
  ImGuiItemFlags_HasSelectionUserData = 1
                                        << 21, // false     // Set by SetNextItemSelectionUserData()
...
enum ImGuiItemStatusFlags_ {
  ...
  ImGuiItemStatusFlags_Visible =
      1 << 8, // [WIP] Set when item is overlapping the current clipping rectangle (Used internally.
...
enum ImGuiInputTextFlagsPrivate_ {
  ...
  ImGuiInputTextFlags_MergedItem = 1
                                   << 28, // For internal use by TempInputText(), will skip calling
...
enum ImGuiButtonFlagsPrivate_ {
  ...
  ImGuiButtonFlags_PressedOnClickRelease =
      1 << 5, // [Default] return true on click + release on same item <-- this is what the majority
              // of Button are using
  ImGuiButtonFlags_PressedOnClickReleaseAnywhere =
      1 << 6, // return true on click + release even if the release event is not done while hovering
              // the item
...
enum ImGuiSelectableFlagsPrivate_ {
  ...
  ImGuiSelectableFlags_SelectOnNav =
      1 << 21, // (WIP) Auto-select when moved into. This is not exposed in public API as to handle
  ...
  ImGuiSelectableFlags_SpanAvailWidth =
      1 << 24, // Span all avail width even if we declared less for layout purpose. FIXME: We may be
...
struct IMGUI_API ImGuiInputTextState {
  ...
  ImVector<char> InitialTextA;       // value to revert to when pressing Escape = backup of end-user
  ...
  bool CursorFollow; // set when we want scrolling to follow the current cursor position (not
                     // always!)
  bool SelectedAllMouseLock; // after a double-click to select all, we ignore further mouse drags to
                             // update selection
  bool Edited;               // edited this frame
  ImGuiInputTextFlags Flags; // copy of InputText() flags. may be used to check if e.g.
                             // ImGuiInputTextFlags_Password is set.
  bool ReloadUserBuf;        // force a reload of user buf so it may be modified externally. may be
                             // automatic in future version.
  int ReloadSelectionStart;  // POSITIONS ARE IN IMWCHAR units *NOT* UTF-8 this is why this is not
                             // exposed yet.
...
enum ImGuiWindowRefreshFlags_ {
  ...
  ImGuiWindowRefreshFlags_TryToAvoidRefresh =
      1 << 0, // [EXPERIMENTAL] Try to keep existing contents, USER MUST NOT HONOR BEGIN() RETURNING
              // FALSE AND NOT APPEND.
...
struct ImGuiNextItemData {
  ...
  // Non-flags members are NOT cleared by ItemAdd() meaning they are still valid during
  // NavProcessItem()
...
struct ImGuiPopupData {
  ...
  ImGuiID OpenParentId; // Set on OpenPopup(), we need this to differentiate multiple menu sets from
                        // each others (e.g. inside menu bar vs loose menu items)
...
struct ImGuiKeyOwnerData {
  ...
  bool LockUntilRelease; // Reading this key requires explicit owner id (until key is released). Set
                         // by ImGuiInputFlags_LockUntilRelease. When this is true LockThisFrame is
                         // always true as well.
...
enum ImGuiInputFlagsPrivate_ {
  ...
  ImGuiInputFlags_RepeatUntilRelease =
      1 << 4, // Stop repeating when released (default for all functions except Shortcut). This only
  ...
  ImGuiInputFlags_RepeatUntilKeyModsChangeFromNone =
      1 << 6, // Stop repeating when released OR if keyboard mods are leaving the None state. Allows
              // going from Mod+Key to Key by releasing Mod.
  ImGuiInputFlags_RepeatUntilOtherKeyPress = 1 << 7, // Stop repeating when released OR if any other
  ...
  ImGuiInputFlags_LockUntilRelease =
      1 << 21, // Further accesses to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0
               // will NOT accepted for polling). Cleared when the key is released or at end of each
               // frame if key is released.
...
enum ImGuiScrollFlags_ {
  ...
  ImGuiScrollFlags_NoScrollParent =
      1 << 6, // Disable forwarding scrolling to parent window if required to keep item/rect visible
              // (only scroll window the function was applied to).
...
enum ImGuiNavMoveFlags_ {
  ...
  ImGuiNavMoveFlags_AllowCurrentNavId =
      1 << 4, // Allow scoring and considering the current NavId as a move target candidate. This is
              // used when the move source is offset (e.g. pressing PageDown actually needs to send
              // a Up move request, if we are pressing PageDown from the bottom-most item we need to
              // stay in place)
...
enum ImGuiOldColumnFlags_ {
  ...
  ImGuiOldColumnFlags_GrowParentContentsSize =
      1 << 4, // Restore pre-1.51 behavior of extending the parent window contents size but _without
              // affecting the columns width at all_. Will eventually remove.
...
struct ImGuiBoxSelectState {
  ...
  ImRect UnclipRect; // Rectangle where ItemAdd() clipping may be temporarily disabled. Need support
                     // by multi-select supporting widgets.
...
struct ImGuiViewportP : public ImGuiViewport {
  ...
  ImVec2 WorkInsetMin; // Work Area inset locked for the frame. GetWorkRect() always fits within
                       // GetMainRect().
  ImVec2 WorkInsetMax; // "
  ImVec2 BuildWorkInsetMin; // Work Area inset accumulator for current frame, to become next frame's
                            // WorkInset
...
struct ImGuiStackLevelInfo {
  ...
  char Desc[57]; // Arbitrarily sized buffer to hold a result (FIXME: could replace Results[] with a
                 // chunk stream?) FIXME: Now that we added CTRL+C this should be fixed.
...
struct ImGuiContext {
  ...
  float FontBaseSize; // (Shortcut) == IO.FontGlobalScale * Font->Scale * Font->FontSize. Base text
                      // height.
  ...
  ImGuiWindow *HoveredWindowUnderMovingWindow; // Hovered window ignoring MovingWindow. Only set if
                                               // MovingWindow is set.
  ImGuiWindow *HoveredWindowBeforeClear; // Window the mouse is hovering. Filled even with _NoMouse.
                                         // This is currently useful for multi-context compositors.
  ...
  ImGuiID
      DebugHookIdInfo; // Will call core hooks: DebugHookIdInfo() from GetID functions, used by ID
  ...
  bool ActiveIdNoClearOnFocusLoss;   // Disable losing active id if the active id window gets
                                     // unfocused.
  bool ActiveIdHasBeenPressedBefore; // Track whether the active id led to a press (this is to allow
                                     // changing between PressOnClick and PressOnRelease without
                                     // pressing twice). Used by range_select branch.
  bool ActiveIdHasBeenEditedBefore; // Was the value associated to the widget Edited over the course
                                    // of the Active state.
  ...
  ImVector<ImGuiFocusScopeData>
      NavFocusRoute; // Reversed copy focus scope stack for NavId (should contains NavFocusScopeId).
  ...
  ImGuiSelectionUserData
      NavLastValidSelectionUserData; // Last valid data passed to SetNextItemSelectionUser(), or -1.
                                     // For current window. Not reset when focusing an item that
                                     // doesn't have selection data.
  ...
  bool NavMousePosDirty;             // When set we will update mouse position if (io.ConfigFlags &
                         ...
                         // default)
  bool NavDisableHighlight; // When user starts using mouse, we hide gamepad/keyboard highlight (NB:
                            // but they are still available, which is why NavDisableHighlight isn't
                            // always != NavDisableMouseHover)
  ...
  bool NavJustMovedToHasSelectionData; // Copy of move result's InFlags &
                                       // ImGuiItemFlags_HasSelectionUserData). Maybe we should just
                                       // store ImGuiNavItemData.

  // Navigation: Windowing (CTRL+TAB for list, or Menu button + keys or directional pads to
  // move/resize)
  ImGuiKeyChord ConfigNavWindowingKeyNext; // = ImGuiMod_Ctrl | ImGuiKey_Tab (or ImGuiMod_Super |
  ...
  ImGuiID DragDropHoldJustPressedId; // Set when holding a payload just made ButtonBehavior() return
                                     // a press.
  ...
  float ScrollbarClickDeltaToGrabCenter; // When scrolling to mouse location: distance between mouse
                                         // and center of grab box, normalized in parent space.
  float SliderGrabClickOffset;
  float SliderCurrentAccum;     // Accumulated slider delta when using navigation controls.
  bool SliderCurrentAccumDirty; // Has the accumulated slider delta changed since last time we tried
                                // to apply it?
  ...
  int LogDepthToExpandDefault; // Default/stored value for LogDepthMaxExpand if not specified in the
                               // LogXXX function call.
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  bool IsSameLine;
  bool IsSetPos;
  ImVec1 Indent; // Indentation / start position from left of window (increased by TreePush/TreePop,
                 // etc.)
  ImVec1 ColumnsOffset; // Offset to the current column (if ColumnsCurrent > 0). FIXME: This and the
                        // above should be a stack to allow use cases like Tree->Column->Tree. Need
                        // revamp columns API.
  ImVec1 GroupOffset;
  ...
  ImVector<float>
      TextWrapPosStack; // Store text wrap pos to restore (attention: .back() is not == TextWrapPos)
};
...
struct IMGUI_API ImGuiWindow {
  ...
  ImGuiViewportP *Viewport; // Always set in Begin(). Inactive windows may have a NULL value here if
                            // their viewport was discarded.
  ...
  float DecoOuterSizeX1, DecoOuterSizeY1; // Left/Up offsets. Sum of non-scrolling outer decorations
                                          // (X1 generally == 0.0f. Y1 generally = TitleBarHeight +
                                          // MenuBarHeight). Locked during Begin().
  float DecoOuterSizeX2, DecoOuterSizeY2; // Right/Down offsets (X2 generally == ScrollbarSize.x, Y2
                                          // == ScrollbarSizes.y).
  ...
  ImGuiID ChildId; // ID of corresponding item in parent window (for navigation to return from child
                   // window to parent window)
  ImGuiID PopupId; // ID in the popup stack when this window is used as a popup/menu (because we use
                   // generic Name/ID for recycling)
  ImVec2 Scroll;
  ImVec2 ScrollMax;
  ...
  short BeginCount; // Number of Begin() during the current frame (generally 0 or 1, 1+ if appending
                    // via multiple Begin/End pairs)
  ...
  ImRect WorkRect;         // Initially covers the whole scrolling region. Reduced by containers e.g
                   // columns/tables when active. Shrunk by WindowPadding*1.0f on each side. This is
                   // meant to replace ContentRegionRect over time (from 1.71+ onward).
  ImRect
      ParentWorkRect; // Backup of WorkRect before entering a container such as columns/tables. Used
                      // by e.g. SpanAllColumns functions to easily access. Stacked containers are
                      // responsible for maintaining this. // FIXME-WORKRECT: Could be a stack?
  ...
  ImVec2ih HitTestHoleSize; // Define an optional rectangular hole where mouse will pass-through the
                            // window.
  ...
  int SettingsOffset; // Offset into SettingsWindows[] (offsets are always valid as we only grow the
                      // array from the back)
  ...
  ImGuiWindow *ParentWindow; // If we are a child _or_ popup _or_ docked window, this is pointing to
                             // our parent. Otherwise NULL.
  ImGuiWindow *ParentWindowInBeginStack;
  ImGuiWindow *RootWindow; // Point to ourself or first ancestor that is not a child window. Doesn't
                           // cross through popups/dock nodes.
  ...
  ImGuiWindow *NavLastChildNavWindow; // When going to the menu bar, we remember the child window we
                                      // came from. (This could probably be made implicit if we kept
                                      // g.Windows sorted by last focused including child window.)
...
struct ImGuiTableColumn {
  ImGuiTableColumnFlags Flags; // Flags after some patching (not directly same as provided by user).
...
struct IMGUI_API ImGuiTable {
  ...
  ImGuiTableFlags SettingsLoadedFlags; // Which data were loaded from the .ini file (e.g. when order
                                       // is not altered we won't save order)
  ...
  ImS16
      InstanceCurrent; // Count of BeginTable() calls with same ID in the same frame (generally 0).
                       ...
                       // table with same ID look are multiple tables, they are just synched.
  ...
  ImRect Bg0ClipRectForDrawCmd; // Actual ImDrawCmd clip rect for BG0/1 channel. This tends to be ==
                                // OuterWindow->ClipRect at BeginTable() because output in BG0/BG1
                                // is cpu-clipped
  ...
  bool IsContextPopupOpen; // Set when default context menu is open (also see: ContextPopupColumn,
                           // InstanceInteracted).
  bool DisableDefaultContextMenu; // Disable default context menu contents. You may submit your own
                                  // using TableBeginContextMenuPopup()/EndPopup()
  ...
  bool HostSkipItems; // Backup of InnerWindow->SkipItem at the end of BeginTable(), because we will
                      // overwrite InnerWindow->SkipItem on a per-column basis
...
struct IMGUI_API ImGuiTableTempData {
  ...
  ImVec1
      HostBackupColumnsOffset; // Backup of OuterWindow->DC.ColumnsOffset at the end of BeginTable()
...
struct ImGuiTableSettings {
  ...
  ImGuiTableColumnIdx
      ColumnsCountMax; // Maximum number of columns this settings instance can store, we can recycle
...
SetItemKeyOwner(ImGuiKey key,
                ImGuiInputFlags flags); // Set key owner to last item if it is hovered or active.
...
IsKeyPressed(ImGuiKey key, ImGuiInputFlags flags,
             ImGuiID owner_id = 0); // Important: when transitioning from old to new IsKeyPressed():
...
IMGUI_API bool
SetShortcutRouting(ImGuiKeyChord key_chord, ImGuiInputFlags flags,
...
IMGUI_API bool ArrowButtonEx(const char *str_id, ImGuiDir dir, ImVec2 size_arg,
                             ImGuiButtonFlags flags = 0);
IMGUI_API bool ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2 &image_size,
...
// IM_ASSERT(IsNamedKey(key)); return IsKeyPressed(key, repeat); } // Removed in 1.87: Mapping from
// named key is always identity!
