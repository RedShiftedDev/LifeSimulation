...
//   overlayed ButtonBehavior() with same ID and different MouseButton (see #8030). You can fix it
//   by:
...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered, bool *out_held,
  ...
  if (g.ActiveId == id) {
    if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
      ...
      if (g.NavActivateDownId == id)
        held =
            true; // hovered == true not true as we are already likely hovered on direct activation.
      else
        ClearActiveID();
    }
    if (pressed)
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow *window, ImGuiAxis axis) {
  ...
  if (axis == ImGuiAxis_X)
    ...
    return ImRect(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size),
                  inner_rect.Min.y, outer_rect.Max.x - border_size, inner_rect.Max.y - border_size);
}
...
bool ImGui::ScrollbarEx(const ImRect &bb_frame, ImGuiID id, ImGuiAxis axis, ImS64 *p_scroll_v,
  ...
  const float grab_h_pixels =
      ImClamp(scrollbar_size_v * ((float)size_visible_v / (float)win_size_v), style.GrabMinSize,
              scrollbar_size_v);
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2 &image_size, const ImVec2 &uv0,
  ...
  // Render
  if (border_size > 0.0f)
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(border_col), 0.0f, ImDrawFlags_None,
                              border_size);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2 &image_size,
...
// Note that ImageButton() adds style.FramePadding*2.0f to provided size. This is in order to
// facilitate fitting an image in a button.
bool ImGui::ImageButton(const char *str_id, ImTextureID user_texture_id, const ImVec2 &image_size,
                        const ImVec2 &uv0, const ImVec2 &uv1, const ImVec4 &bg_col,
  ...
  return ImageButtonEx(window->GetID(str_id), user_texture_id, image_size, uv0, uv1, bg_col,
                       tint_col);
}
...
bool ImGui::BeginComboPreview() {
  ...
  IM_ASSERT(
      ...
      g.LastItemData.Rect.Min.y ==
          preview_data->PreviewRect.Min.y); // Didn't call after BeginCombo/EndCombo block or forgot
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type, void *p_data,
  ...
  void *p_data_default = (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasRefVal)
...
static float STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState *obj, int line_start_idx, int char_idx) {
  unsigned int c;
  ImTextCharFromUtf8(&c, obj->TextA.Data + line_start_idx + char_idx,
                     obj->TextA.Data + obj->TextA.Size);
  if ((ImWchar)c == '\n')
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow *r, ImGuiInputTextState *obj,
  ...
  const ImVec2 size = InputTextCalcTextSize(obj->Ctx, text + line_start_idx, text + obj->CurLenA,
...
static void stb_textedit_replace(ImGuiInputTextState *str, STB_TexteditState *state,
  ...
  IM_ASSERT(0); // Failed to insert character, normally shouldn't happen because of how we currently
                // use stb_textedit_replace()
}
...
static bool InputTextFilterCharacter(ImGuiContext *ctx, unsigned int *p_char,
  ...
  if (c < 0x20) {
    ...
    pass |= (c == '\n') && (flags & ImGuiInputTextFlags_Multiline) !=
                               0; // Note that an Enter KEY will emit \r and be ignored (we poll for
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf, int buf_size,
  ...
  if (is_multiline) {
    ...
    // Prevent NavActivation from Tabbing when our widget accepts Tab inputs: this allows cycling
    // through widgets without stopping.
  ...
  if ((init_state && g.ActiveId != id) || init_changed_specs || init_reload_from_user_buf) {
    ...
    if (recycle_state &&
        (state->CurLenA != buf_len || (strncmp(state->TextA.Data, buf, buf_len) != 0)))
      recycle_state = false;
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    // operations for other code (e.g. calling window trying to use CTRL+A and CTRL+B: formet would
    ...
    if (IsKeyPressed(ImGuiKey_LeftArrow)) {
      ...
      if (g.PlatformIO.Platform_SetClipboardTextFn != NULL) {
        ...
        state->TextA.Data[ie] =
            0; // A bit of a hack since SetClipboardText only takes null terminated strings
  ...
  if (g.ActiveId == id) {
    ...
    if (revert_edit && !is_readonly) {
      if (flags & ImGuiInputTextFlags_EscapeClearsAll) {
        ...
        // operation itself
        value_changed = true;
        stb_textedit_replace(state, state->Stb, state->InitialTextA.Data,
    ...
    // ImGuiInputTextFlags_EnterReturnsTrue would fail. This also allows the user to use InputText()
    // with ImGuiInputTextFlags_EnterReturnsTrue without maintaining any user-side storage (please
    // note that if you use this property along ImGuiInputTextFlags_CallbackResize you can end up
    // with your temporary string object unnecessarily allocating once a frame, either store your
    // string data, either if you don't then don't use ImGuiInputTextFlags_CallbackResize).
        ...
        if (event_flag) {
          ...
          if (buf_dirty) {
            ...
            InputTextReconcileUndoStateAfterUserCallback(
                ...
                callback_data.BufTextLen); // FIXME: Move the rest of this block inside function and
                                           // rename to InputTextReconcileStateAfterUserCallback() ?
  ...
  if (render_cursor || render_selection) {
...

    {
      ...
      const char *selmin_ptr =
          render_selection ? text_begin + ImMin(state->Stb->select_start, state->Stb->select_end)
                           : NULL;
      ...
      if (selmin_line_no >= 0) {
        select_start_offset.x =
            InputTextCalcTextSize(&g, ImStrbol(selmin_ptr, text_begin), selmin_ptr).x;
    ...
    // FIXME-OPT: Multiline could submit a smaller amount of contents to AddText() since we already
    // iterated through it.
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState *state) {
  ...
  Text("CurLenA: %d, Cursor: %d, Selection: %d..%d", state->CurLenA, stb_state->cursor,
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label,
  ...
  ImGuiID storage_id =
      (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasStorageID) ? g.NextItemData.StorageId : id;
    ...
    if (pressed && g.DragDropHoldJustPressedId != id) {
      ...
      if (flags & ImGuiTreeNodeFlags_OpenOnArrow)
        toggled |= is_mouse_x_over_arrow &&
                   !g.NavDisableMouseHover; // Lightweight equivalent of IsMouseHoveringRect() since
...
bool ImGui::Selectable(const char *label, bool selected, ImGuiSelectableFlags flags,
  ...
  if (!is_visible)
    if (!is_multi_select || !g.BoxSelectState.UnclipMode ||
        !g.BoxSelectState.UnclipRect.Overlaps(
            bb)) // Extra layer of "no logic clip" for box-select support (would be more overhead to
                 // add to ItemAdd)
      return false;
  ...
  if (is_visible) {
    ...
    if (highlighted || selected) {
      ...
      if (selected && !highlighted)
        col = GetColorU32(ImLerp(GetStyleColorVec4(ImGuiCol_Header),
    ...
    if (g.NavId == id) {
      ImGuiNavHighlightFlags nav_highlight_flags =
          ImGuiNavHighlightFlags_Compact | ImGuiNavHighlightFlags_NoRounding;
      if (is_multi_select)
        nav_highlight_flags |= ImGuiNavHighlightFlags_AlwaysDraw; // Always show the nav rectangle
...
ImGuiMultiSelectIO *ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size,
  ...
  if (flags & ImGuiMultiSelectFlags_ScopeWindow) // Mark parent child window as navigable into, with
                                                 // highlight. Assume user will always submit
                                                 // interactive items.
        ...
        (ms->RangeSrcPassedBy == false &&
         ms->IO.RangeSrcItem !=
             ImGuiSelectionUserData_Invalid)) // Can't read storage->RangeSrcItem here -> we want
                                              // the state at begining of the scope (see tests for
                                              // easy failure)
    {
    ...
    if (ImGuiBoxSelectState *bs = GetBoxSelectState(ms->BoxSelectId)) {
      ...
      if ((rect_overlap_curr && !rect_overlap_prev && !selected) ||
        ...
        if (storage->LastSelectionSize <= 0 && bs->IsStartedSetNavIdOnce) {
          pressed = true; // First item act as a pressed: code below will emit selection request and
                          // set NavId (whatever we emit here will be overridden anyway)
...
static void ImGuiSelectionBasicStorage_BatchSetItemSelected(ImGuiSelectionBasicStorage *selection,
  ...
  if (selected && !is_contained)
    storage->Data.push_back(
        ImGuiStoragePair(id, selection_order)); // Push unsorted at end of vector, will be sorted in
                                                // SelectionMultiAmendsFinish()
...
// subtleties with capturing info from the label, but for 99% uses it could essentially be rewritten
// as:
//    if (ImGui::BeginChild("...", ImVec2(ImGui::CalcItemWidth(), ImGui::GetTextLineHeight()
//    * 7.5f), ImGuiChildFlags_FrameStyle))
//        { .... }
//    ImGui::EndChild();
//    ImGui::SameLine();
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled) {
  ...
  if (!enabled) // explicitly close if an open menu becomes disabled, facilitate users code a lot in
                // pattern such as 'if (BeginMenu("options", has_object)) { ..use object.. }'
    want_close = true;
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar *tab_bar, ImGuiID tab_id,
  ...
  ImGuiContext &g = *GImGui;
  float margin =
      g.FontSize * 1.0f; // When to scroll to make Tab N+1 visible always make a bit of N visible to
...
bool ImGui::BeginTabItem(const char *label, bool *p_open, ImGuiTabItemFlags flags) {
  ...
  IM_ASSERT(!(flags & ImGuiTabItemFlags_Button)); // BeginTabItem() Can't be used with button flags,
                                                  // use TabItemButton() instead!

  bool ret = TabItemEx(tab_bar, label, p_open, flags, NULL);
  if (ret && !(flags & ImGuiTabItemFlags_NoPushId)) {
    ImGuiTabItem *tab = &tab_bar->Tabs[tab_bar->LastTabItemIdx];
    PushOverrideID(tab->ID); // We already hashed 'label' so push into the ID stack directly instead
                             // of doing another hash through PushID(label)
  }
  return ret;
}
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open,
  ...
  if (tab_contents_visible && (tab_bar->Flags & ImGuiTabBarFlags_DrawSelectedOverline) &&
    ...
    display_draw_list->AddLine(bb.GetTL() + ImVec2(x_offset, y_offset),
  ...
  // seems ok) (We test IsItemHovered() to discard e.g. when another item is active or drag and drop
  // over the tab bar, which g.HoveredId ignores)
