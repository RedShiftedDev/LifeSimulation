...
#pragma clang diagnostic ignored "-Wunsafe-buffer-usage" // warning: 'xxx' is an unsafe pointer used
                                                         // for buffer access
...
typedef int ImGuiPopupFlags;       // -> enum ImGuiPopupFlags_      // Flags: for
...
IMGUI_API void EndFrame();        // ends the Dear ImGui frame. automatically called by Render(). If
...
IMGUI_API void
ShowDemoWindow(bool *p_open = NULL); // create Demo window. demonstrate most ImGui
...
IMGUI_API void
ShowDebugLogWindow(bool *p_open = NULL); // create Debug Log window. display a simplified log
...
IMGUI_API bool
ShowStyleSelector(const char *label); // add style selector block (not a window), essentially
...
IMGUI_API const char *GetVersion(); // get the compiled version string e.g. "1.80 WIP" (essentially
...
IMGUI_API bool IsWindowHovered(
    ImGuiHoveredFlags flags = 0); // is current window hovered and hoverable (e.g. not blocked by a
                                  ...
                                  // to Dear ImGui or to your underlying app, you should not use
                                  // this function! Use the 'io.WantCaptureMouse' boolean for that!
                                  // Refer to FAQ entry "How can I tell whether to dispatch
...
IMGUI_API ImVec2 GetWindowPos();   // get current window position in screen space (IT IS UNLIKELY
                                   // YOU EVER NEED TO USE THIS. Consider always using
                                   // GetCursorScreenPos() and GetContentRegionAvail() instead)
IMGUI_API ImVec2 GetWindowSize();  // get current window size (IT IS UNLIKELY YOU EVER NEED TO USE
                                   // THIS. Consider always using GetCursorScreenPos() and
                                   // GetContentRegionAvail() instead)
IMGUI_API float GetWindowWidth();  // get current window width (IT IS UNLIKELY YOU EVER NEED TO
...
IMGUI_API void SetNextWindowBgAlpha(float alpha); // set next window background color alpha. helper
                                                  // to easily override the Alpha component of
                                                  // ImGuiCol_WindowBg/ChildBg/PopupBg. you may also
                                                  // use ImGuiWindowFlags_NoBackground.
IMGUI_API void
SetWindowPos(const ImVec2 &pos,
             ImGuiCond cond = 0); // (not recommended) set current window position - call
...
IMGUI_API void
SetWindowFontScale(float scale); // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you
...
IMGUI_API void SetWindowFocus(const char *name);       // set named window to be focused / top-most.
...
SetScrollFromPosX(float local_x,
                  float center_x_ratio = 0.5f); // adjust scrolling amount to make given position
                                                // visible. Generally GetCursorStartPos() + offset
                                                // to compute a valid position.
IMGUI_API void
SetScrollFromPosY(float local_y,
                  float center_y_ratio = 0.5f); // adjust scrolling amount to make given position
                                                // visible. Generally GetCursorStartPos() + offset
                                                // to compute a valid position.
...
IMGUI_API void
SetNextItemWidth(float item_width); // set width of the _next_ common large "item+label"
                                    // widget. >0.0f: width in pixels, <0.0f align xx pixels
                                    // to the right of window (so -FLT_MIN always align width
                                    // to the right side)
IMGUI_API float CalcItemWidth();    // width of item given pushed settings and current cursor
...
// Try not to use it.
IMGUI_API ImVec2 GetCursorScreenPos(); // cursor position, absolute coordinates. THIS IS YOUR
                                       // BEST FRIEND (prefer using this rather than
                                       // GetCursorPos(), also more useful to work with
                                       // ImDrawList API).
IMGUI_API void SetCursorScreenPos(const ImVec2 &pos); // cursor position, absolute coordinates.
...
IMGUI_API ImVec2 GetCursorStartPos(); // [window-local] initial cursor position, in window-local
...
TextUnformatted(const char *text,
                const char *text_end = NULL); // raw text without formatting. Roughly equivalent to
...
IMGUI_API void Bullet(); // draw a small circle + keep the cursor on the same line. advance
...
IMGUI_API void SetColorEditOptions(
    ImGuiColorEditFlags flags); // initialize current options (generally on application startup)
...
IMGUI_API bool TreeNodeExV(const void *ptr_id, ImGuiTreeNodeFlags flags, const char *fmt,
                           va_list args) IM_FMTLIST(3);
IMGUI_API void TreePush(const char *str_id); // ~ Indent()+PushID(). Already called by
...
IMGUI_API bool CollapsingHeader(
    ...
    ImGuiTreeNodeFlags flags = 0); // when 'p_visible != NULL': if '*p_visible==true' display an
                                   // additional small close button on upper right of the header
                                   // which will set the bool to false when clicked, if
...
IMGUI_API void SetNextItemStorageID(ImGuiID storage_id); // set id to use for open/close storage
                                                         // (default to same as item id).
...
IMGUI_API bool Selectable(
    ...
    const ImVec2 &size = ImVec2(0, 0)); // "bool selected" carry the selection state (read-only).
...
IMGUI_API bool IsItemToggledSelection(); // Was the last item selection state toggled? Useful
...
IMGUI_API void SetTooltip(const char *fmt, ...)
    IM_FMTARGS(1); // set a text-only tooltip. Often used after a ImGui::IsItemHovered()
...
IMGUI_API void OpenPopupOnItemClick(
    ...
    ImGuiPopupFlags popup_flags = 1); // helper to open popup when clicked on last item. Default to
                                      // ImGuiPopupFlags_MouseButtonRight == 1. (note: actually
                                      // triggers on the mouse _released_ event to be consistent
                                      // with popup behaviors)
...
IMGUI_API bool TableNextColumn(); // append into the next column (or first column of next row
                                  // if currently in last column). Return true when column is
                                  // visible.
IMGUI_API bool TableSetColumnIndex(int column_n); // append into the specified column. Return
...
IMGUI_API void TableAngledHeadersRow(); // submit a row with angled headers for every column
...
IMGUI_API ImGuiTableSortSpecs *TableGetSortSpecs(); // get latest sort specs for the table (NULL if
                                                    // not sorting).  Lifetime: don't hold on this
                                                    // pointer over multiple frames or past any
...
IMGUI_API int TableGetHoveredColumn(); // return hovered column. return -1 when table is not
...
IMGUI_API void
SetTabItemClosed(const char *tab_or_docked_window_label); // notify TabBar or Docking system of a
...
IMGUI_API bool
BeginDragDropSource(ImGuiDragDropFlags flags = 0); // call after submitting an item which may be
                                                   // dragged. when this return true, you can call
                                                   // SetDragDropPayload() + EndDragDropSource()
IMGUI_API bool
SetDragDropPayload(const char *type, const void *data, size_t sz,
                   ImGuiCond cond = 0); // type is a user defined string of maximum 32 characters.
...
IMGUI_API void
SetKeyboardFocusHere(int offset = 0); // focus keyboard on the next widget. Use positive 'offset'
...
IMGUI_API bool IsItemActive();    // is the last item active? (e.g. button being held, text field
...
IMGUI_API bool IsItemEdited();      // did the last item modify its underlying value this frame? or
...
IMGUI_API ImDrawList *GetBackgroundDrawList(); // this draw list will be the first rendered one.
                                               // Useful to quickly draw shapes/text behind dear
                                               // imgui contents.
IMGUI_API ImDrawList *GetForegroundDrawList(); // this draw list will be the last rendered one.
...
IMGUI_API bool
IsKeyChordPressed(ImGuiKeyChord key_chord); // was key chord (mods + key) pressed, e.g. you can pass
...
IMGUI_API int GetKeyPressedAmount(ImGuiKey key, float repeat_delay,
                                  float rate); // uses provided repeat rate/delay. return a count,
                                               // most often 0 or 1 but might be >1 if RepeatRate is
...
IMGUI_API void SetItemKeyOwner(ImGuiKey key); // Set key owner to last item ID if it is hovered or
...
IMGUI_API bool IsMouseReleased(ImGuiMouseButton button); // did mouse button released? (went
                                                         // from Down to !Down)
IMGUI_API bool IsMouseDoubleClicked(
    ImGuiMouseButton button); // did mouse button double-clicked? Same as
...
IMGUI_API bool IsAnyMouseDown(); // [WILL OBSOLETE] is any mouse button held? This was designed
...
IMGUI_API ImVec2 GetMousePosOnOpeningCurrentPopup(); // retrieve mouse position at the time of
                                                     // opening popup we have BeginPopup() into
                                                     // (helper to avoid user backing that value
                                                     // themselves)
IMGUI_API bool
IsMouseDragging(ImGuiMouseButton button,
                float lock_threshold = -1.0f); // is mouse dragging? (uses io.MouseDraggingThreshold
                                               // if lock_threshold < 0.0f)
IMGUI_API ImVec2 GetMouseDragDelta(
    ImGuiMouseButton button = 0,
    float lock_threshold = -1.0f); // return the delta from the initial clicking position while the
...
IMGUI_API void LoadIniSettingsFromDisk(
    const char *ini_filename); // call after CreateContext() and before the first call
...
IMGUI_API void
SaveIniSettingsToDisk(const char *ini_filename); // this is automatically called (if io.IniFilename
                                                 // is not empty) a few seconds after any
                                                 // modification that should be reflected in the
                                                 // .ini file (and also by DestroyContext).
IMGUI_API const char *SaveIniSettingsToMemory(
    size_t *out_ini_size = NULL); // return a zero-terminated string with the .ini data which you
...
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_NoScrollWithMouse = 1
                                       << 4, // Disable user vertically scrolling with mouse wheel.
  ...
  ImGuiWindowFlags_NoFocusOnAppearing = 1 << 12, // Disable taking focus when transitioning from
  ...
  ImGuiWindowFlags_AlwaysHorizontalScrollbar = 1 << 15, // Always show horizontal scrollbar (even if
                                                        // ContentSize.x < Size.x)
  ImGuiWindowFlags_NoNavInputs = 1 << 16, // No gamepad/keyboard navigation within the window
  ImGuiWindowFlags_NoNavFocus = 1 << 17,  // No focusing toward this window with gamepad/keyboard
...
// Obsolete names
  ...
  ImGuiWindowFlags_NavFlattened = 1 << 31, // Obsoleted in 1.90.9: Use ImGuiChildFlags_NavFlattened
                                           // in BeginChild() call.
#endif
};
...
enum ImGuiChildFlags_ {
  ...
  ImGuiChildFlags_AutoResizeX = 1 << 4, // Enable auto-resizing width. Read "IMPORTANT: Size
                                        // measurement" details above.
  ImGuiChildFlags_AutoResizeY = 1 << 5, // Enable auto-resizing height. Read "IMPORTANT: Size
                                        // measurement" details above.
  ImGuiChildFlags_AlwaysAutoResize = 1
                                     << 6, // Combined with AutoResizeX/AutoResizeY. Always measure
                                           // size even when child is hidden, always return true,
  ...
  ImGuiChildFlags_NavFlattened = 1 << 8, // [BETA] Share focus scope, allow gamepad/keyboard
...
enum ImGuiItemFlags_ {
  ...
  ImGuiItemFlags_AutoClosePopups = 1 << 4, // true     // MenuItem()/Selectable() automatically
                                           // close their parent popup window.
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_CharsScientific = 1
                                        << 2, // Allow 0123456789.+-*/eE (Scientific notation input)
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_OpenOnArrow = 1 << 7, // Open when clicking on the arrow part (default for
  ...
  ImGuiTreeNodeFlags_Bullet = 1 << 9, // Display a bullet instead of arrow. IMPORTANT: node can
                                      // still be marked open/close if you don't set the _Leaf flag!
  ImGuiTreeNodeFlags_FramePadding =
      1 << 10, // Use FramePadding (even for an unframed text node) to vertically
  ...
  ImGuiTreeNodeFlags_SpanTextWidth = 1 << 13,  // Narrow hit box + narrow hovering highlight, will
                                               // only cover the label text.
  ImGuiTreeNodeFlags_SpanAllColumns = 1 << 14, // Frame will span all columns of its container table
...
enum ImGuiPopupFlags_ {
  ...
  ImGuiPopupFlags_NoReopen =
      1 << 5, // For OpenPopup*(), BeginPopupContext*(): don't reopen same popup
  ...
  ImGuiPopupFlags_NoOpenOverItems = 1 << 8, // For BeginPopupContextWindow(): don't return true when
...
enum ImGuiTabBarFlags_ {
  ...
  ImGuiTabBarFlags_Reorderable = 1 << 0, // Allow manually dragging tabs to re-order them + New tabs
...
enum ImGuiTabItemFlags_ {
  ...
  ImGuiTabItemFlags_UnsavedDocument = 1 << 0, // Display a dot next to the title + set
...
enum ImGuiHoveredFlags_ {
  ImGuiHoveredFlags_None = 0, // Return true if directly over the item/window, not obstructed by
                              // another window, not obstructed by an active popup or modal blocking
                              // inputs under them.
  ImGuiHoveredFlags_ChildWindows = 1 << 0, // IsWindowHovered() only: Return true if any children of
  ...
  ImGuiHoveredFlags_NoPopupHierarchy = 1
                                       << 3, // IsWindowHovered() only: Do not consider popup
                                             // hierarchy (do not treat popup emitter as parent of
  ...
  ImGuiHoveredFlags_NoNavOverride = 1 << 11, // IsItemHovered() only: Disable using gamepad/keyboard
  ...
  ImGuiHoveredFlags_ForTooltip = 1 << 12, // Shortcut for standard flags when using IsItemHovered()
                                          // + SetTooltip() sequence.
  ...
  ImGuiHoveredFlags_DelayNone = 1 << 14, // IsItemHovered() only: Return true immediately (default).
...
enum ImGuiDragDropFlags_ {
  ...
  ImGuiDragDropFlags_SourceNoHoldToOpenOthers = 1
                                                << 2, // Disable the behavior that allows to open
                                                      // tree nodes and collapsing header by holding
  ...
  ImGuiDragDropFlags_PayloadNoCrossProcess = 1 << 7, // Hint to specify that the payload may not be
                                                     // copied outside current process.
  ...
  ImGuiDragDropFlags_AcceptNoDrawDefaultRect = 1 << 11, // Do not draw the default highlight
                                                        // rectangle when hovering over target.
...
enum ImGuiInputFlags_ {
  ...
  ImGuiInputFlags_Repeat = 1 << 0, // Enable repeat. Return true on successive repeats. Default for
  ...
  ImGuiInputFlags_RouteFocused = 1 << 11, // Route to windows in the focus stack (DEFAULT).
                                          // Deep-most focused window takes inputs. Active item
                                          // takes inputs over deep-most focused window.
  ImGuiInputFlags_RouteGlobal = 1 << 12,  // Global route (unless a focused window
  ...
  ImGuiInputFlags_RouteOverActive =
      1 << 15, // Option: global route: higher priority than active item. Unlikely
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NoKeyboard = 1 << 6, // Instruct dear imgui to disable keyboard inputs and
                                        // interactions. This is done by ignoring keyboard events
...
enum ImGuiBackendFlags_ {
  ...
  ImGuiBackendFlags_HasSetMousePos = 1
                                     << 2, // Backend Platform supports io.WantSetMousePos requests
                                           // to reposition the OS mouse position (only used if
                                           // ImGuiConfigFlags_NavEnableSetMousePos is set).
  ImGuiBackendFlags_RendererHasVtxOffset = 1
                                           << 3, // Backend Renderer supports ImDrawCmd::VtxOffset.
                                                 // This enables output of large meshes (64K+
...
enum ImGuiColorEditFlags_ {
  ...
  ImGuiColorEditFlags_NoAlpha = 1 << 1,  //              // ColorEdit, ColorPicker, ColorButton:
                                         //              ignore Alpha component (will only read 3
                                         //              components from the input pointer).
  ImGuiColorEditFlags_NoPicker = 1 << 2, //              // ColorEdit: disable picker when clicking
                                         //              on color square.
  ...
  ImGuiColorEditFlags_NoSmallPreview = 1
                                       << 4, //              // ColorEdit, ColorPicker: disable
                                             //              color square preview next to the
                                             //              inputs. (e.g. to show only the inputs)
  ImGuiColorEditFlags_NoInputs = 1 << 5, //              // ColorEdit, ColorPicker: disable inputs
  ...
  ImGuiColorEditFlags_NoSidePreview = 1
                                      << 8, //              // ColorPicker: disable bigger color
                                            //              preview on right side of the picker, use
  ...
  ImGuiColorEditFlags_NoBorder = 1 << 10, //              // ColorButton: disable border (which is
                                          //              enforced by default)
  ...
  ImGuiColorEditFlags_AlphaPreview = 1
                                     << 17, //              // ColorEdit, ColorPicker, ColorButton:
                                            //              display preview as a transparent color
  ...
  ImGuiColorEditFlags_HDR = 1 << 19, //              // (WIP) ColorEdit: Currently only disable
                                     //              0.0f..1.0f limits in RGBA edition (note: you
                                     //              probably want to use ImGuiColorEditFlags_Float
                                     //              flag as well).
  ImGuiColorEditFlags_DisplayRGB = 1 << 20, // [Display]    // ColorEdit: override _display_ type
                                            // among RGB/HSV/Hex. ColorPicker: select any
  ...
  ImGuiColorEditFlags_PickerHueBar = 1 << 25,   // [Picker]     // ColorPicker: bar
                                                // for Hue, rectangle for Sat/Value.
  ImGuiColorEditFlags_PickerHueWheel = 1 << 26, // [Picker]     // ColorPicker: wheel for Hue,
                                                // triangle for Sat/Value.
  ImGuiColorEditFlags_InputRGB = 1 << 27,       // [Input]      // ColorEdit, ColorPicker: input and
                                                // output data in RGB format.
  ImGuiColorEditFlags_InputHSV = 1 << 28,       // [Input]      // ColorEdit, ColorPicker: input and
                                                // output data in HSV format.
...
enum ImGuiSliderFlags_ {
  ...
  ImGuiSliderFlags_NoRoundToFormat = 1 << 6, // Disable rounding underlying value to match precision
                                             // of the display format string (e.g. %.3f values are
...
enum ImGuiTableFlags_ {
  ...
  ImGuiTableFlags_NoSavedSettings =
      1 << 4, // Disable persisting columns order, width and sort settings in
              // the .ini file.
  ImGuiTableFlags_ContextMenuInBody = 1 << 5, // Right-click on columns body/contents will display
                                              // table context menu. By default it is available in
                                              // TableHeadersRow(). Decorations
  ImGuiTableFlags_RowBg = 1 << 6,             // Set each RowBg color with ImGuiCol_TableRowBg or
  ...
  ImGuiTableFlags_SizingStretchSame =
      4 << 13, // Columns default to _WidthStretch with default weights all equal,
  ...
  ImGuiTableFlags_NoHostExtendY = 1
                                  << 17, // Make outer height stop exactly at outer_size.y (prevent
  ...
  ImGuiTableFlags_NoClip = 1 << 20,    // Disable clipping rectangle for every individual columns
...
template <typename T> struct ImVector {
  ...
  inline void push_back(const T &v) {
    ...
    memcpy((void *)&Data[Size], &v, sizeof(v));
    Size++;
  }
...
struct ImGuiStyle {
  ...
  float TabMinWidthForCloseButton;    // Minimum width for close button to appear on
  ...
  float HoverDelayShort;      // Delay for IsItemHovered(ImGuiHoveredFlags_DelayShort).
                              // Usually used along with HoverStationaryDelay.
  float HoverDelayNormal;     // Delay for
                              // IsItemHovered(ImGuiHoveredFlags_DelayNormal). "
  ImGuiHoveredFlags HoverFlagsForTooltipMouse; // Default flags when using
                                               // IsItemHovered(ImGuiHoveredFlags_ForTooltip)
                                               // or BeginItemTooltip()/SetItemTooltip() while
                                               // using mouse.
  ImGuiHoveredFlags HoverFlagsForTooltipNav;   // Default flags when using
...
struct ImGuiIO {
  ...
  const char *LogFilename;        // = "imgui_log.txt"// Path to .log file (default parameter
  ...
  float MouseDoubleClickTime;    // = 0.30f          // Time for a double-click, in
                                 // seconds.
  float MouseDoubleClickMaxDist; // = 6.0f           // Distance threshold to stay
  ...
  IMGUI_API void AddMouseWheelEvent(float wheel_x,
                                    float wheel_y); // Queue a mouse wheel update. wheel_y<0: scroll
                                                    // down, wheel_y>0: scroll up, wheel_x<0: scroll
                                                    // right, wheel_x>0: scroll left.
  IMGUI_API void AddMouseSourceEvent(ImGuiMouseSource source); // Queue a mouse source change
  ...
  // moving anywhere)
  ImVec2 MousePos;              // Mouse position, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX)
...
struct ImGuiPayload {
  ...
  char DataType[32 + 1];  // Data type tag (short user-supplied string, 32
                          // characters max)
  bool Preview;           // Set when AcceptDragDropPayload() was called and mouse has been
...
struct ImGuiListClipper {
  ...
  IMGUI_API bool Step(); // Call until it returns false. The DisplayStart/DisplayEnd fields
  ...
  IMGUI_API void IncludeItemsByIndex(int item_begin,
...
enum ImGuiMultiSelectFlags_ {
  ...
  ImGuiMultiSelectFlags_SingleSelect =
      1 << 0, // Disable selecting more than one item. This is available to allow
  ...
  ImGuiMultiSelectFlags_NoAutoClear =
      1 << 4, // Disable clearing selection when navigating or selecting another
  ...
  ImGuiMultiSelectFlags_NoAutoClearOnReselect =
      1 << 5, // Disable clearing selection when clicking/selecting an already
              // selected item.
  ImGuiMultiSelectFlags_BoxSelect1d = 1
                                      << 6, // Enable box-selection with same width and same x pos
                                            // items (e.g. full row Selectable()). Box-selection
                                            // works better with little bit of spacing between items
  ...
  ImGuiMultiSelectFlags_ScopeWindow = 1
                                      << 11, // Scope for _BoxSelect and _ClearOnClickVoid is whole
                                             // window (Default). Use if BeginMultiSelect() covers a
  ...
  ImGuiMultiSelectFlags_SelectOnClick = 1 << 13, // Apply selection on mouse down when clicking on
                                                 // unselected item. (Default)
...
struct ImGuiMultiSelectIO {
  //------------------------------------------// BeginMultiSelect /
  // EndMultiSelect
  ImVector<ImGuiSelectionRequest> Requests; //  ms:w, app:r     /  ms:w  app:r   // Requests to
                                            //  apply to your selection data.
  ImGuiSelectionUserData RangeSrcItem; //  ms:w  app:r     /                // (If using clipper)
  ...
  ImGuiSelectionUserData NavIdItem;    //  ms:w, app:r     /                // (If using deletion)
...
struct ImGuiSelectionRequest {
  ...
  ImGuiSelectionUserData RangeFirstItem; //                  /  ms:w, app:r   // Parameter for
                                         //                  SetRange request (this is generally ==
                                         //                  RangeSrcItem when shift selecting from
                                         //                  top to bottom).
  ImGuiSelectionUserData RangeLastItem;  //                  /  ms:w, app:r   // Parameter for
                                         //                  SetRange request (this is generally ==
                                         //                  RangeSrcItem when shift selecting from
...
struct ImGuiSelectionBasicStorage {
  ...
  IMGUI_API void
  ApplyRequests(ImGuiMultiSelectIO *ms_io); // Apply selection requests coming from
                                            // BeginMultiSelect() and EndMultiSelect() functions. It
  ...
  IMGUI_API bool
  GetNextSelectedItem(void **opaque_it,
                      ImGuiID *out_id); // Iterate selection with 'void* it = NULL; ImGuiId id;
...
enum ImDrawFlags_ {
  ...
  ImDrawFlags_RoundCornersTopLeft = 1 << 4,  // AddRect(), AddRectFilled(), PathRect(): enable
                                             // rounding top-left corner only (when rounding > 0.0f,
                                             // we default to all corners). Was 0x01.
  ImDrawFlags_RoundCornersTopRight = 1 << 5, // AddRect(), AddRectFilled(), PathRect(): enable
                                             // rounding top-right corner only (when rounding >
                                             // 0.0f, we default to all corners). Was 0x02.
  ImDrawFlags_RoundCornersBottomLeft = 1 << 6,  // AddRect(), AddRectFilled(), PathRect(): enable
                                                // rounding bottom-left corner only (when rounding >
                                                // 0.0f, we default to all corners). Was 0x04.
  ImDrawFlags_RoundCornersBottomRight = 1 << 7, // AddRect(), AddRectFilled(), PathRect(): enable
                                                // rounding bottom-right corner only (when rounding
                                                // > 0.0f, we default to all corners). Wax 0x08.
  ImDrawFlags_RoundCornersNone = 1 << 8, // AddRect(), AddRectFilled(), PathRect(): disable rounding
                                         // on all corners (when rounding > 0.0f). This is NOT zero,
...
enum ImDrawListFlags_ {
  ...
  ImDrawListFlags_AntiAliasedFill = 1 << 2, // Enable anti-aliased edge around filled shapes
...
struct ImDrawList {
  ...
  IMGUI_API void AddDrawCmd(); // This is useful if you need to forcefully create a new draw
...
struct ImDrawData {
  ...
  ImVec2 FramebufferScale;         // Amount of pixels for each unit of DisplaySize. Based
  ...
  IMGUI_API void AddDrawList(ImDrawList *draw_list); // Helper to add an external draw list
                                                     // into an existing ImDrawData.
  IMGUI_API void DeIndexAllBuffers(); // Helper to convert all buffers from indexed to
  ...
  IMGUI_API void
  ScaleClipRects(const ImVec2 &fb_scale); // Helper to scale the ClipRect field of each
...
struct ImFontConfig {
  ...
  const ImWchar *GlyphRanges;    // NULL     // THE ARRAY DATA NEEDS TO PERSIST AS LONG AS
...
struct ImFontAtlas {
  ...
  IMGUI_API ImFont *AddFontFromMemoryTTF(
      ...
      const ImWchar *glyph_ranges = NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas!
  ...
  IMGUI_API ImFont *AddFontFromMemoryCompressedBase85TTF(
      ...
      const ImFontConfig *font_cfg = NULL,
      const ImWchar *glyph_ranges = NULL); // 'compressed_font_data_base85' still owned by caller.
                                           // Compress with binary_to_compressed_c.cpp with -base85
                                           // parameter.
  IMGUI_API void ClearInputData(); // Clear input data (all ImFontConfig structures including
  ...
  IMGUI_API const ImWchar *GetGlyphRangesChineseFull(); // Default + Half-Width + Japanese
  ...
  const ImFontBuilderIO *FontBuilderIO; // Opaque interface to a font builder (default to
...
struct ImFont {
  ...
  ImVector<float> IndexAdvanceX; // 12-16 // out //            // Sparse. Glyphs->AdvanceX
  ...
  float Ascent, Descent;          // 4+4   // out //            // Ascent: distance from
                                  // top to bottom of e.g. 'A' [0..FontSize] (unscaled)
...
IMGUI_API ImVec2 GetContentRegionMax();       // Content boundaries max (e.g. window boundaries
...
IMGUI_API ImVec2 GetWindowContentRegionMin(); // Content boundaries min for the window (roughly
                                              // (0,0)-Scroll), in window-local coordinates. You
                                              // should never need this. Always use
                                              // GetCursorScreenPos() and
                                              // GetContentRegionAvail()!
IMGUI_API ImVec2 GetWindowContentRegionMax(); // Content boundaries max for the window (roughly
...
IMGUI_API ImGuiKey GetKeyIndex(ImGuiKey key); // Map ImGuiKey_* values into legacy native key index.
